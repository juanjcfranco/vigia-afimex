...
import { nivelAlertaPorDias, buildMailtoUrl } from '@/lib/business-logic';

interface AlertaSinMovimientoModalProps {
  open: boolean;
  onClose: () => void;
...
export default function AlertaSinMovimientoModal({
  open,
  onClose,
  guiasSeleccionadas,
  contactos,
  onCompletado,
}: AlertaSinMovimientoModalProps) {
  ...
  function enviarCorreoOficina(oficina: string, lista: Guia[]) {
    ...
    const lineas = lista.map((g, i) => {
      const alerta = nivelAlertaPorDias(g.dias_sin_movimiento);
      return `${i + 1}. Guía: ${g.guia} | Desc: ${g.descripcion || '—'} | Estado: ${g.estado_guia || '—'} | Destino: ${
        g.oficina_destino || '—'
      } | Últ. Mov: ${g.f_historia || '—'} | Días sin mov: ${g.dias_sin_movimiento ?? '—'} | ${alerta.etiqueta}`;
    });

    const cuerpoTexto = [
      'Estimado equipo,',
      '',
      `Las siguientes guías en la Oficina ${oficina} presentan días sin movimiento y requieren seguimiento inmediato.`,
      '',
      ...lineas,
      '',
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl max-w-2xl w-full max-h-[88vh] overflow-y-auto vg-scroll p-6">
        <div className="flex items-center justify-between mb-1">
          <h2 className="font-bold text-lg">⏰ Alerta de Guías sin Movimiento</h2>
                    ...
                    <td>
                      {enviado ? (
                        ...
                        <button
                          ...
                          className="text-[11px] font-semibold text-white rounded-md px-2.5 py-1 disabled:opacity-30 bg-[#EA7C1A]"
                        >
                          Enviar
                        </button>
                      )}
        ...
        {registrado && (
          <div className="text-[12px] text-[var(--vg-green)] font-semibold mb-3">
            ✅ Alertas registradas en el historial
          </div>
        )}
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="text-[12px] font-semibold text-white rounded-md px-3 py-1.5 disabled:opacity-50 bg-[#EA7C1A]"
            >
